...
class AndroidController extends Controller {	
    ...
    public function all_floor_count(){
    	...
    	$floor_names =M('room_info','','DB_Local')->distinct(true)->field('floor_id')->select();
	...
	public function roomName_map($room_id)
{
	...
	public function all_newest_info(){
		$floor_name_list=['F1','F2','F3','F4','F5','F6','YZ '];
		$return_SsInfo=array();
		$if_ask_all=$_POST['askALL'];
		if($if_ask_all=="all"){
			$rooms_info=M('room_info','','DB_Local')->order('floor_id asc')->select();
		}else{
	...
	public function app_set_threshold()
	 {
		 ...
		 $setInfo['tem_high_threshold']=$POST['tempMax'];
		 $setInfo['tem_low_threshold']=$POST['tempMin'];
		 $setInfo['hum_high_threshold']=$POST['humiMax'];
		 $setInfo['hum_low_threshold']=$POST['humiMin'];
		 $ifSetALL=$POST['ifALL'];
		 
		 $findInfo=M('room_info','','DB_Local')->where($condition_room)->limit(1)->find();
		 if($findInfo)
		 {
