...
public function send_SsInfo_web()
	{
		$floor_name_list=['一楼','二楼','三楼','四楼','五楼','六楼','七楼','八楼','九楼','十楼'];
		$ifSendEmail=0;
		$warnSendContext='';
		$return_SsInfo=array();
		$condition_floor_cy['floor_id'] = array('LT','7');//$condition_floor_cy['floor_id'] = array(array('gt',0),array('lt',6));
		$rooms_info=M('room_info','','DB_Local')->where($condition_floor_cy)->select();//找出共有多少个房间
		
		$last_warn_time=M('room_info','','DB_Local')->order('alarm_time desc')->find();//先找出最近报警时间，避免本次更新影响
		if($last_warn_time){
			if($last_warn_time['alarm_time']==null){
				$last_warn_time['alarm_time']=date('Y-m-d').' '.(date('H')-2).":00:00";
			}
		}
		else{
			$last_warn_time['alarm_time']=date('Y-m-d').' '.(date('H')-2).":00:00";
			...
			if($SsInfo){
				for ($i=0; $i <count($SsInfo); $i++) {
					...
					if($result_find){
						...
						//如果离线
						if(D('Alarm')->DifferentDateTime(date("Y-m-d H:i:s"),$result_find['datetime'],10)){
							$SsInfo[$i]['online']=0;
						...
						//查询每类节点的对应值
							...
							if($SsInfo[$i]['type']=='tem_hum'){
								...
								if($SsInfo[$i]['temp_value']>$rooms_info[$r_id]['tem_high_threshold'] || $SsInfo[$i]['temp_value']<$rooms_info[$r_id]['tem_low_threshold'] || $SsInfo[$i]['humi_value']>$rooms_info[$r_id]['hum_high_threshold'] ||$SsInfo[$i]['humi_value']<$rooms_info[$r_id]['hum_low_threshold'])
								{
									...
									{
										$updateInfo['tem_alarm_datetime']=$SsInfo[$i]['datetime'];
									...
									{
										$updateInfo['hum_alarm_datetime']=$SsInfo[$i]['datetime'];
										$warnSendContext_floor=$warnSendContext_floor.'湿度超过阈值;';
									}
									$updateInfo['alarm_time']=date("Y-m-d H:i:s");
									M('room_info','','DB_Local')->where($condition_room)->save($updateInfo);//并更新报警记录表里面该楼层的报警时间
				...
				if($warnSendContext_floor!=null){
				$warnSendContext=$warnSendContext.'楼层编号:'.$condition_room['floor_id'].'房间编号:'.$condition_room['room_id'].'<br>'.$warnSendContext_floor;
				$warnSendContext_floor=null;
				}
			}
		}
		...
		if($ifSendEmail==1){
			if(D('Alarm')->DifferentDateTime(date("Y-m-d H:i:s"),$last_warn_time['alarm_time'],30)){//时间计算不对
				$sendEmail=D('Mail');
...
// 		$last_warn_time=M('room_info','','DB_Local')->order('alarm_time desc')->limit(1)->find();//先找出最近报警时间，避免本次更新影响
...
/**************返回历史信息到界面***返回每层楼的****************/
	...
	{
		$floors_info=array();$floor_count=7;
