						...
						if(data[i].online==0)//该节点离线
						{
							...
							if (data[i].type=='tem_hum') {
								...
								if(data[i].status==1){
										...
										+color+';width:60px;height:40px;border-radius:15px;cursor: pointer;" onclick="graph('
										+data[i].floor_id+',\"'+data[i].room_id+'\")">'+content+'</div>';
										$(address).append(html);
									}
								else{
									...
									';width: 60px;height: 60px;border-radius: 50% 50%;cursor: pointer;line-height:28px;" onclick="graph('
									+data[i].floor_id+","+data[i].room_id+')">'+content+'</div>';
									$(address).append(html);
								...
								if (ifStartGraph==1 && graphRM_id==data[i].room_id && graphFL_id==data[i].floor_id && firstFlag==1) {//如果有让开始画图
									firstFlag=0;
									//alert("tem_hum");
									tem_hum_value_array[0].push(data[i].time);
									tem_hum_value_array[1].push(Number(data[i].temp_value));//温度
									tem_hum_value_array[2].push(Number(data[i].humi_value));//湿度
								}
							}
							else{
